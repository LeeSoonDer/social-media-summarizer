...
版本：0.8.2 · 分支 `feat/sidepanel` · 更新于 2026-09-15
...
4. 画面支持多屏追加合并去重；扫描前先裁到内容区，识别后按行的置信度过闸；
   设置页、笔记导出、全量备份已就绪。
5. 口播（Phase 5）已取消不做，分区改为显示平台字幕并引导去「画面」抓烧录字幕。

## 已完成（OCR 降噪 · 2026-09-15）

用户反馈「扫出来一堆杂质」。改了三处，见 decisions 同日那条：

- **先取景再截图**：`content.js` 新增 `ocrTarget` 消息，按 视频 > 主图 > 平台正文容器
  挑出这一屏的内容矩形（CSS 像素），侧栏拿到后换算成截图的设备像素并裁剪。
  导航栏、侧栏推荐、点赞数、关注按钮根本不进模型。
  取景失败、裁得过小、坐标越界，一律退回扫整屏——不会因为取景挂掉扫描。
- **置信度闸门**：`recognize` 改成要 `blocks`，按每行 `confidence < 60` 丢弃。
  全被滤光时退回全文（宁可多也别交白卷）。
- **行级过滤加两条**：符号占比 < 50% 的行、单字重复行（「口口口口」「|||||」）。

顺带减负：裁剪后画布通常只剩原来的四分之一（整屏 2x 在 1920×1080 上要 33MB 像素），
识别也更快；画布用完立刻 `width = height = 0` 回收。

toast 会写明这次「只扫了视频区域」还是「扫的是整屏」，取景取歪时一眼能看出来。

**还没在真实浏览器里验过**：语法检查、裁剪坐标（7 种屏幕配置）、过滤规则（14 条）
都在 node 里跑过，但取景在真实社媒页面上准不准，要用户 Load unpacked 实测。
...
2. **「跟着视频扫」**：一个开关，打开后自动抓字幕，关了才停。两条路：
   页面有字幕 DOM（YouTube 的 `.ytp-caption-segment`、B 站）就挂 MutationObserver 静默订阅，
   零 OCR、零错字；没有就定时抽帧 OCR 抓烧录字幕，复用 Phase 4 管线 + 本次的裁剪与闸门。
   约 150 行、零新权限，是口播的低成本替代。
   硬限制：`captureVisibleTab` 只拍当前可见标签页且有配额（约 2 次/秒），
   所以视频必须一直在屏幕上播，间隔取 1~1.5 秒。字幕带要按相似度去重，不能精确匹配
   （同一句连着好几帧都在，而且 OCR 每帧都在抖）。
   前置条件（裁剪 + 置信度闸门）已于 2026-09-15 完成。
