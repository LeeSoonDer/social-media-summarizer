...
## 2026-09-10 - 口播（Phase 5）取消，不做录音转写
Decision: 不做 tabCapture + offscreen + 本地 Whisper。口播分区保留，只显示平台已提供的字幕轨道；没有字幕时引导用户去「画面」用「再扫一屏」抓烧录字幕。
Reason: 用户在评估后决定「往不麻烦、不贵、现实实际、高效的方向走」。管线本身照官方样板不难，真正的代价在模型：CLAUDE.md 禁止把权重打进 git（仓库已 100MB），所以要运行时下载 40–250MB；中文准确率要能用基本得上 small（250MB）；MV3 禁远程代码，ONNX 运行时的 wasm 还得本地打包；WASM 上跑是数倍实时，一条 60 秒短视频要等一两分钟；外加音频重采样分块管线和两个敏感权限。为覆盖「完全没有字幕的纯口播视频」这一类内容，代价与收益不成比例。
Tradeoff: 无字幕的纯口播视频（部分 Reels talking head）抓不到文字。中文短视频大多有烧录字幕，Phase 4 的多屏 OCR 已能覆盖。
Future Implications: `lib/format.js` 的 `speechOf()` 仍然优先读 `content.speech`，接点留着。将来若要补，优先做「跟着视频扫」（定时抽帧 OCR，复用 Phase 4 管线，约 150 行、零新权限），而不是 Whisper。

## 2026-09-10 - 选择器修复需要真实 DOM 快照，不盲改
Decision: Phase 6 没有改动任何平台选择器。改为在每次提取的「提取说明」第一条写明正文来自哪条策略（平台选择器 / og-meta / 可见文字兜底）。
Reason: 小红书、IG、TikTok、X 都在登录墙后面，我看不到真实 DOM。凭空改选择器有很大概率把现在能用的弄坏，而验证不了。
Tradeoff: CLAUDE.md Phase 6 的「按真实常用站修选择器」这一项没有完成，只做了让退化可见。
Future Implications: 用户在出问题的页面 F12 复制 outerHTML 存成文件给我，才能真正修，并按架构 §7.4 建立 fixture 回归。

## 2026-09-11 - 打开侧栏不再自动跑 OCR
Decision: 打开侧栏只抽文本（几百毫秒）。画面文字改为在「画面」分区点「扫本屏」（或按 Alt+Shift+S）才跑。同一 URL 已扫过的屏继续沿用，不会因为重新采集而丢失。
Reason: 架构文档 §4.3 写的是「打开即采标题 + 正文 + 视口 OCR」。实际用下来，OCR 是整个产品最重的动作——首次要装 31MB 语言包，之后每次也要 2-5 秒——而多数时候用户只是想看正文或翻一眼笔记。让用户为自己没要求的事情买单，是最典型的交互反模式，也是这个产品最大的摩擦点。用户在 2026-09-11 明确拍板改掉。
Tradeoff: 图文帖多一次点击。但纯文字帖（X、Reddit、Threads）从此完全不用等 OCR，而它们占多数。
Future Implications: 架构文档 §4.3 的「打开即采」这一句以本条为准。以后往默认路径上加任何耗时动作，都要先问「用户这次真的要它吗」。

## 2026-09-11 - 反馈分两层：顶部状态栏说状态，底部 toast 说结果
Decision: 顶部状态栏只表示「本页处于什么状态」（正在提取 / 已采集 N 屏 / 失败）；动作的结果（已复制 / 已导出 / 已删除 / 已存进收集箱）走底部 toast，钉在底栏正上方。笔记分区的全部反馈都走 toast。
Reason: 反馈应该出现在动作发生的地方。原来所有提示都写在顶部那一行，用户在笔记列表最底下点「导出」，提示出现在屏幕最顶端，很可能根本看不到，会以为没反应。
Tradeoff: 多一个常驻 DOM 元素。
Future Implications: 新增任何动作按钮，反馈一律走 notify()，不要往 setStatus() 里塞。setStatus 现在有配套的 setPageStatus/restorePageStatus——耗时过程写进度可以用 setStatus，但结束时必须还原，否则状态栏会停在「正在识别文字」看起来像卡死。

## 2026-09-15 - OCR 降噪：先裁内容区，再按行的置信度过闸
Decision: 扫描前先问 `content.js` 要这一屏的取景矩形（视频 > 主图 > 平台正文容器），截图裁到这块再放大送 Tesseract；识别改用 `recognize(img, {}, { text: true, blocks: true })`，按每行的 `confidence` 过闸（阈值 60），闸门把整屏滤空时退回全文。行级过滤新增「符号占比 < 50% 的行」与「单字重复行」两条。
Reason: 用户反馈「扫出来一堆杂质」。杂质其实是两类：一类是读对了但不想要的（导航栏、侧栏推荐、点赞数、关注按钮），一类是真乱码（图标、头像、渐变被当成字）。前者只能靠不喂进去解决——纯事后过滤分不清「OCR 认错的字」和「本来就长这样的字」；后者 Tesseract 自己其实给了把握度，之前只取 `data.text` 等于把这个信号整个扔了。
Tradeoff: 取景可能取歪（比如页面用一张全屏背景图），所以 `runOcr` 回执里带 `cropped` 和 `label`，toast 会写明「只扫了视频区域」还是「扫的是整屏」，让用户一眼看出是取景问题还是真没字。置信度阈值写死 60，没做成设置项；真要调再说。
Future Implications: 顺带解决了内存：原来是整屏 2x（1920×1080 的屏要开 3840×2160 ≈ 33MB 像素画布），现在通常只剩四分之一，画布用完立刻置 0 回收。这条是「跟着视频扫」的前置——自动抓帧必须先有裁剪和闸门，否则只是把杂质自动化地灌进来。语言那一项（eng 模型对着中文硬猜出拉丁串）没单独做，因为置信度闸门已经吃掉了大部分。
